...
- **Query-vocabulary drift** (are incoming questions drifting toward
  ...
  considered and deliberately left out: a real drift metric needs
  clustering over real traffic this project doesn't have yet, and
  `EMBEDDING_MODEL` has never changed, so a drift metric for it would
  always read zero — shipping either would look scientific without
  being useful yet.
